...
const EditUserDetails = ({ user }) => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then(({ data }) => {
        setUserData(data);
        console.log("1. Edittutee deets", data);
        console.log("this is the userData", userData); //correct
      })
      .catch((error) => {
  ...
  const handleUpdateUserDetails = (values) => {
    ...
    const url = urlcat(SERVER, `/user/edituserdetails/${userData._id}`);
    axios
      .put(url, values)
      .then(({ data }) => {
        console.log("2. Edited tutee deets", data);
        setUserData(values);
      ...
      .catch((error) => {
        if (
          ...
          console.log("User details could not be updated.");
          // error page
        }
      });
  };
